...
export function StarButton({
  ...
  return (
    <button
      ...
      ref={pathRef}
      className={cn(
        "relative z-3 cursor-pointer overflow-hidden h-10 px-4 py-2 inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-3xl text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50 group/star-button",
        className,
      )}
      {...props}
    >
      ...
      {/* Hover Full Border Effect */}
      <div
        className="absolute inset-0 rounded-[inherit] opacity-0 group-hover/star-button:opacity-100 transition-opacity duration-500 pointer-events-none z-5"
        style={{

import Link from "next/link";
import Image from "next/image";
import { Mail, Phone, MapPin, ArrowUpRight, Instagram } from "lucide-react";

const NAV_LINKS = [
  { label: "Home", href: "/" },
  { label: "Über Uns", href: "/uber-uns" },
  { label: "Leistungen", href: "/leistungen" },
  { label: "Kontakt", href: "/contact" },
];

const LEGAL_LINKS = [
  { label: "Impressum", href: "/impressum" },
  { label: "Datenschutz", href: "/privacy" },
];

export const Footer = () => {
  return (
    <footer className="relative bg-[#111111] text-white overflow-hidden">
      {/* Large background brand text */}
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 pointer-events-none select-none">
        <span className="text-[20vw] font-black uppercase tracking-tighter text-white/[0.02] leading-none whitespace-nowrap">
          SAXO
        </span>
      </div>

      <div className="container mx-auto px-6 md:px-12 relative z-10">
        {/* Main Footer Content */}
        <div className="pt-20 pb-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
          {/* Col 1: Brand & Tagline */}
          <div className="lg:col-span-1 flex flex-col gap-6">
            <Link href="/" className="flex items-center gap-3 w-fit">
              <Image
                src="/saho-logo.png"
                alt="SaxoHandwerk Logo"
                width={44}
                height={44}
                className="object-contain"
              />
              <span className="text-2xl font-black uppercase tracking-tighter">
                Saxo<span className="text-[#f55733]">Handwerk</span>
              </span>
            </Link>
            <p className="text-white/50 font-sans text-sm leading-relaxed max-w-xs">
              Tradition trifft Moderne. Ihr verlässlicher Partner für
              Komplettsanierungen und erstklassiges Handwerk in Dresden und
              Sachsen.
            </p>

            {/* Social Icons */}
            <div className="flex items-center gap-3 mt-2">
              <a
                href="#"
                className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center hover:bg-[#f55733] hover:border-[#f55733] transition-all duration-300 group"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
                  height="16"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                  className="text-white/60 group-hover:text-white transition-colors"
                >
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                </svg>
              </a>
              <a
                href="#"
                className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center hover:bg-[#f55733] hover:border-[#f55733] transition-all duration-300 group"
              >
                <Instagram
                  size={16}
                  className="text-white/60 group-hover:text-white transition-colors"
                />
              </a>
            </div>
          </div>

          {/* Col 2: Navigation */}
          <div className="flex flex-col gap-5">
            <h4 className="text-xs font-bold uppercase tracking-[0.3em] text-[#f55733]">
              Navigation
            </h4>
            <nav className="flex flex-col gap-3">
              {NAV_LINKS.map((link) => (
                <Link
                  key={link.label}
                  href={link.href}
                  className="text-white/60 hover:text-white transition-colors font-sans text-sm flex items-center gap-2 group w-fit"
                >
                  {link.label}
                  <ArrowUpRight className="w-3 h-3 opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200" />
                </Link>
              ))}
            </nav>
          </div>

          {/* Col 3: Contact */}
          <div className="flex flex-col gap-5">
            <h4 className="text-xs font-bold uppercase tracking-[0.3em] text-[#f55733]">
              Kontakt
            </h4>
            <div className="flex flex-col gap-4">
              <a
                href="tel:+49123456789"
                className="flex items-center gap-3 text-white/60 hover:text-white transition-colors font-sans text-sm w-fit"
              >
                <Phone className="h-4 w-4 text-[#f55733] shrink-0" />
                +49 (0) 123 456 789
              </a>
              <a
                href="mailto:kontakt@saxo-handwerk.de"
                className="flex items-center gap-3 text-white/60 hover:text-white transition-colors font-sans text-sm w-fit"
              >
                <Mail className="h-4 w-4 text-[#f55733] shrink-0" />
                kontakt@saxo-handwerk.de
              </a>
              <div className="flex items-start gap-3 text-white/60 font-sans text-sm">
                <MapPin className="h-4 w-4 text-[#f55733] shrink-0 mt-0.5" />
                <span>
                  Musterstraße 123
                  <br />
                  01067 Dresden, Deutschland
                </span>
              </div>
            </div>
          </div>

          {/* Col 4: CTA */}
          <div className="flex flex-col gap-5">
            <h4 className="text-xs font-bold uppercase tracking-[0.3em] text-[#f55733]">
              Projekt starten
            </h4>
            <p className="text-white/50 font-sans text-sm leading-relaxed">
              Kostenlose Erstberatung — unverbindlich und persönlich. Wir freuen
              uns auf Ihr Projekt.
            </p>
            <Link
              href="/contact"
              className="inline-flex items-center gap-2 bg-[#f55733] hover:bg-[#d43d1a] text-white font-bold font-sans text-xs uppercase tracking-wider px-5 py-3 rounded-full transition-colors duration-300 w-fit"
            >
              Kontakt aufnehmen
              <ArrowUpRight className="w-4 h-4" />
            </Link>
          </div>
        </div>

        {/* Divider */}
        <div className="h-px bg-white/10 w-full" />

        {/* Bottom Bar */}
        <div className="py-8 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-white/30 font-sans text-xs">
            &copy; {new Date().getFullYear()} SaxoHandwerk. Alle Rechte
            vorbehalten.
          </p>

          <div className="flex items-center gap-6">
            {LEGAL_LINKS.map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className="text-white/30 hover:text-white/60 transition-colors font-sans text-xs"
              >
                {link.label}
              </Link>
            ))}
          </div>

          <p className="text-white/30 font-sans text-xs">
            Created by{" "}
            <a
              href="#"
              className="text-white/50 hover:text-white transition-colors"
            >
              Viktor Gladkich
            </a>
          </p>
        </div>
      </div>
    </footer>
  );
};
